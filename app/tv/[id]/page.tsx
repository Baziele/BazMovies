...
export default function TvShowDetails({ params }: { params: { id: string } }) {
  // Find the TV show by ID
  const show = allMovies.find((m) => m.id === params.id && m.type === "tv")

  // Find similar shows (same genre)
  const similarShows = allMovies
    .filter((m) => m.id !== params.id && m.genre === show?.genre && m.type === "tv")
    .slice(0, 6)
  ...
  const showMedia = [
    { type: "image", url: "/placeholder.svg?height=720&width=1280", alt: "Show scene 1" },
    { type: "image", url: "/placeholder.svg?height=720&width=1280", alt: "Show scene 2" },
    { type: "image", url: "/placeholder.svg?height=720&width=1280", alt: "Show scene 3" },
    { type: "image", url: "/placeholder.svg?height=720&width=1280", alt: "Behind the scenes" },
    { type: "image", url: "/placeholder.svg?height=720&width=1280", alt: "Show poster" },
  ]

  const nextImage = () => {
    setCurrentImage((prev) => (prev === showMedia.length - 1 ? 0 : prev + 1))
  }

  const prevImage = () => {
    setCurrentImage((prev) => (prev === 0 ? showMedia.length - 1 : prev - 1))
  }
  ...
  return (
    <main className="min-h-screen bg-background">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="flex-1">
                ...
                >
                  ...
                  <span className="text-white/80 text-sm">{show.year}</span>
                  <span className="text-white/80 text-sm">{show.duration}</span>
                </div>
                  ...
                  <div className="flex items-center mr-4">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        ...
                        className={cn(
                          "h-5 w-5 mr-0.5",
                          i < Math.floor(show.rating / 2) ? "text-yellow-400 fill-yellow-400" : "text-gray-400",
                        )}
                      />
                    ))}
                  </div>
                  <span className="text-white font-medium">{show.rating}/10</span>
                </div>
                ...
                <div className="flex flex-wrap gap-3 animate-fade-in-up" style={{ animationDelay: "0.5s" }}>
                  <Button size="lg" asChild className="group animate-pulse-subtle">
                    <Link href={`/watch/${show.id}/1/1`}>
                      <Play className="mr-2 h-4 w-4 group-hover:animate-ping-once" /> Watch First Episode
                    </Link>
                  </Button>
          ...
          <div className="space-y-6">
            ...
            {show.director && (
              <div className="animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
                <h3 className="font-semibold text-muted-foreground mb-1">Creator</h3>
                <p className="text-lg">{show.director}</p>
              </div>
            )}
            ...
            {show.studio && (
              <div className="animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
                <h3 className="font-semibold text-muted-foreground mb-1">Network</h3>
                <p>{show.studio}</p>
              </div>
            )}
          ...
          <div className="space-y-6">
            ...
            <div className="border rounded-lg p-4 space-y-4 hover:shadow-md transition-shadow animate-fade-in">
              ...
              <div className="flex justify-between">
                ...
                <span>{seasons.reduce((total, season) => total + season.episodes.length, 0)}</span>
              </div>
              ...
              {show.filmingLocations && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {show.filmingLocations.map((location, index) => (
                      <span
                        key={index}
      ...
      <section className="container py-8 border-t">
        ...
        <div className="space-y-6">
          {seasons.map((season) => (
            <div key={season.seasonNumber} className="border rounded-lg overflow-hidden animate-fade-in-up">
              <button
                className="w-full flex items-center justify-between p-4 bg-muted/30 hover:bg-muted/50 transition-colors"
                onClick={() => toggleSeason(season.seasonNumber)}
              >
                <h3 className="text-xl font-bold flex items-center">Season {season.seasonNumber}</h3>
                      ...
                      <div className="flex flex-col md:flex-row">
                        <div className="relative md:w-64 h-40">
                          ...
                          <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-start p-3">
                            <Button asChild size="sm" variant="secondary">
                              <Link href={`/watch/${show.id}/${season.seasonNumber}/${episode.episodeNumber}`}>
                        ...
                        <div className="p-4 flex-1">
                          <div className="flex items-center justify-between mb-2">
                            <h4 className="font-bold">Episode {episode.episodeNumber}</h4>
        ...
        <div className="space-y-8">
          <div className="animate-fade-in-up">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
              {show.cast.length > 0
                ? show.cast.map((cast) => <CastMember key={cast.id} cast={cast} />)
                : [...Array(12)].map((_, i) => (
                    <CastMember
                      key={i}
                      cast={{
          ...
          <div className="animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                <h4 className="font-medium text-muted-foreground mb-1">Creator</h4>
                <p className="font-bold">{show.director || "Creator Name"}</p>
              </div>

              <div className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                <h4 className="font-medium text-muted-foreground mb-1">Writers</h4>
                <p className="font-bold">{show.writers || "Writer Names"}</p>
              </div>

              <div className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                <h4 className="font-medium text-muted-foreground mb-1">Executive Producer</h4>
      ...
      <section className="container py-8 border-t">
        ...
        <div className="animate-fade-in-up">
          ...
          <div className="mt-6 text-center">
            <Button asChild>
              <Link href={`/search?genre=${show.genre}&type=tv`} className="flex items-center gap-2">
                See more {show.genre} TV shows <ChevronRight className="h-4 w-4" />
              </Link>
            </Button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center">
          <div className="relative w-full max-w-6xl mx-auto">
            ...
            <div className="relative aspect-video">
              <Image
                src={showMedia[currentImage].url || "/placeholder.svg"}
                alt={showMedia[currentImage].alt}
                fill
