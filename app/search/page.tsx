...
import { Search, Filter, X, Sparkles, Film, Calendar, Star, Clock } from "lucide-react"
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    if (contentType === "movie") {
      results = results.filter((item) => item.type === "movie" || !item.type) // Default to movie if type not specified
    ...
    if (selectedGenres.length > 0) {
      results = results.filter((movie) => selectedGenres.includes(movie.genre))
    }

    // Apply year range filter
    results = results.filter((movie) => movie.year >= yearRange[0] && movie.year <= yearRange[1])

    // Apply rating filter
    results = results.filter((movie) => movie.rating >= ratingRange[0] && movie.rating <= ratingRange[1])
  ...
  }, [searchQuery, selectedGenres, yearRange, ratingRange, sortBy, contentType])
    ...
    setTimeout(() => {
      ...
      const results = allMovies
        .filter((movie) => {
          ...
          if (prompt.includes("action") && genre.includes("action")) return true
          if (prompt.includes("drama") && genre.includes("drama")) return true
          if (prompt.includes("comedy") && genre.includes("comedy")) return true
          if (prompt.includes("sci-fi") && (genre.includes("sci-fi") || genre.includes("science fiction"))) return true

          // Check for time period keywords
          if (prompt.includes("80s") && movie.year >= 1980 && movie.year < 1990) return true
          if (prompt.includes("90s") && movie.year >= 1990 && movie.year < 2000) return true
          if (prompt.includes("2000s") && movie.year >= 2000 && movie.year < 2010) return true

          // Check for plot elements in description
          if (description.includes(prompt) || title.includes(prompt)) return true

          // Check for specific themes
          if (prompt.includes("space") && (description.includes("space") || title.includes("space"))) return true
          if (prompt.includes("love") && (description.includes("love") || description.includes("romance"))) return true
          if (prompt.includes("war") && (description.includes("war") || title.includes("war"))) return true
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <Button
            ...
            className={cn(
              ...
              aiMode ? "border-purple-500 text-purple-500 hover:bg-purple-500/10" : "",
            )}
          >
            {aiMode ? (
              <>
        ...
        {aiMode ? (
            ...
            <div className="bg-card border rounded-lg p-6 mb-6">
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-2 text-purple-500 mb-2">
                  ...
                  <h2 className="text-xl font-medium">Describe what you're looking for</h2>
                </div>
          ...
          <>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ...
              {!showFilters && (
                <Button variant="ghost" size="sm" className="text-sm" onClick={() => setShowFilters(true)}>
                  More Filters...
                </Button>
              )}
            </div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {(ratingRange[0] > 0 || ratingRange[1] < 10) && (
                  <Badge variant="secondary" className="flex items-center gap-1">
                    {ratingRange[0]} - {ratingRange[1]} Rating
                    <X className="h-3 w-3 cursor-pointer" onClick={() => setRatingRange([0, 10])} />
                  </Badge>
                )}
                ...
                {(yearRange[0] !== 1970 || yearRange[1] !== 2025) && (
                  <Badge variant="secondary" className="flex items-center gap-1">
                    ...
                    <X className="h-3 w-3 cursor-pointer" onClick={() => setYearRange([1970, 2025])} />
                  </Badge>
                )}
