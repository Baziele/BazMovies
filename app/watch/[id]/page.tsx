...
export default function WatchMovie({ params }: { params: { id: string } }) {
  ...
  const movie = allMovies.find((m) => m.id === params.id && m.type !== "tv")

  // Find similar movies (same genre)
  const similarMovies = allMovies
    .filter((m) => m.id !== params.id && m.genre === movie?.genre && m.type !== "tv")
    .slice(0, 4)
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="container py-8">
        <div className="flex items-center gap-2 mb-4">
          <Link href={`/movie/${movie.id}`} className="text-sm text-primary hover:underline flex items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-8">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
              ...
              <span className="bg-primary/10 text-primary px-2 py-1 rounded text-sm">{movie.genre}</span>
            ...
            <div className="space-y-6 mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {movie.language && (
                  <div>
                    <h3 className="font-semibold text-muted-foreground mb-1">Language</h3>
                    <p>{movie.language}</p>
                  </div>
                )}
              </div>
